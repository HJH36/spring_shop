...
<%
   ...
   String strParams = "&current_page="+current_page;
   ...
   order_list = OD.OrderList(current_page, searchtitle, searchstring, gainCounter);
   ...
   //전체 개수
   int total_count = OD.OrderTotal(searchtitle, searchstring);
   ...
   //총 판매 금액
	int total_order_price = OD.OrderTotalPrice(state);
...
<body>
   ...
   <div class="container">
     ...
     <form name="SearchForm" method="post">
      ...
      <div style="text-align:right;margin:10px; font-weight:900; color:blue;">
         총 판매 금액 : <%=df.format(total_order_price)%> 원
      </div>
                           ...
                           <tr>
                              ...
                              <td style = "text-align:center; vertical-align:middle;">
                              	<div style="margin:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_state('<%=order_idx%>', 'A', 'S')" style = "width:80px;"><i class="fab fa-cc-visa"></i>결제완료</button>
                              	</div>
                              	<div style="margin-bottom:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_state('<%=order_idx%>', 'B', 'S')" style = "width:80px;"><i class="fas fa-box-open"></i>>배송준비</button>
                              	</div>
                              	<div style="margin-bottom:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_state('<%=order_idx%>', 'C', 'S')" style = "width:80px;"><i class="fas fa-truck"></i>배송중</button>
                              	</div>
                              	<div style="margin-bottom:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_state('<%=order_idx%>', 'D', 'S')" style = "width:80px;"><i class="fas fa-home"></i>배송완료</button>
                              	</div>
                              	<div style="margin-bottom:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_state('<%=order_idx%>', 'E', 'S')" style = "width:80px;"><i class="far fa-window-close"></i>주문취소</button>
                              	</div>
                              	<div style="margin-bottom:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_state('<%=order_idx%>', 'F', 'S')" style = "width:80px;"><i class="fas fa-exchange-alt"></i>교환</button>
                              	</div>
                              	<div style="margin-bottom:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_state('<%=order_idx%>', 'G', 'S')" style = "width:80px;"><i class="fas fa-undo-alt"></i>환불</button>
                              	</div>
                              	<div style="margin-bottom:2px;">
                              		<button type="button" class="btn btn-dafault btn-xs" onclick="send_delete('<%=order_idx%>', 'S', '<%=strParams %>')" style = "width:80px;"><i class="fas fa-trash"></i>완전삭제</button>
                              	</div>
                              </td>
                           </tr>
						 ...
						 -->
                          ...
                          <tr>
            			   	<td colspan="8" class="text-right">
            			   		<button type="button" class="btn btn-default btn-sm" onclick="send_state('<%=order_group_idx%>', 'A', 'M')">결제완료</button>
            			   		<button type="button" class="btn btn-default btn-sm" onclick="send_state('<%=order_group_idx%>', 'B', 'M')">배송준비</button>
            			   		<button type="button" class="btn btn-default btn-sm" onclick="send_state('<%=order_group_idx%>', 'C', 'M')">배송중</button>
            			   		<button type="button" class="btn btn-default btn-sm" onclick="send_state('<%=order_group_idx%>', 'D', 'M')">배송완료</button>
            			   		<button type="button" class="btn btn-default btn-sm" onclick="send_state('<%=order_group_idx%>', 'E', 'M')">주문취소</button>
            			   		<button type="button" class="btn btn-default btn-sm" onclick="send_state('<%=order_group_idx%>', 'F', 'M')">교환</button>
            			   		<button type="button" class="btn btn-default btn-sm" onclick="send_state('<%=order_group_idx%>', 'G', 'M')">환불</button>
            			   		<button type="button" class="btn btn-danger btn-sm" onclick="send_delete('<%=order_group_idx%>', 'M', '<%=strParams %>')" style = "width:80px;"><i class="fas fa-trash"></i>완전삭제</button>
            			   	</td>
            			  </tr>
...
<script>
	...
	//주문 삭제하기
	function send_delete(idx_temp, type, strParams) {
		var ans = confirm("정말 삭제하시겠습니까?");
		if(ans){
			location.href = "/order_backend_delete_ok.do?idx_temp=" + idx_temp+"&type=" + type + strParams;
		}else{
			return false;
		}
	...
	function send_state(idx_temp, state, type) {
		location.href = "/order_state_modify.do?idx_temp=" + idx_temp
