...
int current_page = 1;
	...
	BoardBackendDAO boardDAO = new BoardBackendDAO();
...
<body>
			...
			%>
				<tr>
					...
					<td style="padding-left:<%=marginSize+5%>px;">
						<a href="javascript:send_view('<%=current_page %>','<%=board_idx%>')"><%=board_title %></a>
					</td>
	...
	<div class="text-center">
			...
			<nav>
			  <ul class="pagination">
			    <li>
			      <a href="/board_backend_default.do?current_page=1" aria-label="Previous">
				...
				<%for(int i=0;i<total_page;i++){ %>
					<%if(current_page==i+1){ %>
						<li class="active"><a href="/board_backend_default.do?current_page=<%=i+1%>"><%=i+1 %></a></li>
					<%}else{ %>
						<li><a href="/board_backend_default.do?current_page=<%=i+1%>"><%=i+1 %></a></li>
					<%} %>
				<%} %>

			    <li>
			      <a href="/board_backend_default.do?current_page=<%=total_page%>" aria-label="Next">
...
<script>
	...
	function send_write(){
		location.href="/board_backend_write_default.do";
	}
	...
	function send_view(current_page, board_idx){
		location.href="/board_backend_view_default.do?current_page="+current_page+"&board_idx="+board_idx;
	}
